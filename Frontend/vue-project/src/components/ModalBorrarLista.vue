...
const emit = defineEmits(["update:modelValue", "listaBorrada"])
...
const borrarLista = async () => {
  try {
    const response = await api.delete(`/tareas/listas/${props.dato}/`,
    {
      headers: {
...
<template>
  <div class="modal-container" v-if="modelValue" @click.self="closeModal">
    <div class="modal-box">
      <h1>¿SEGURO QUE QUIERES BORRAR ESTA LISTA?</h1>
      <form @submit.prevent="borrarLista">
